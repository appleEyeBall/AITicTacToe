...
public class MinMax {
	 ...
	 * @return
	 */
	public ArrayList<Node> findMoves()
	{		
		...
		if(minOrMax == "max")		//list of all X moves
		{
			...
			for(int x = 0 ; x < initState.length ; x++)
			{
				if(initState[x].equals("b"))								//its a blank space
				{
					...
					Node temp = new Node(copyInit, x+1);				//make a ExpertAlgorithm.Node out of it
	 ...
	 * @param x_or_y
	 */
	public void setStateList_MinMaxValues(ArrayList<Node> theStateList, int x_or_y)
	{
	 ...
	 * @param list
	 */
	public void print_minMax(ArrayList<Node> list)
	{
	...
	/**
	 * Randomly select spots for novice player
	 */
	public Node pickRandomSpot(){
		boolean bool = true;
		Integer val = 0;
		while (bool){
			Random r = new Random();
			val = r.nextInt(9);
			if(initState[val].equals("X") || initState[val].equals("O")){
				bool = true;
			}
			else {
				initState[val] = "X";
